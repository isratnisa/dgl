...
def main(args):
    ...
    with Timer(dev) as t:
        for i in range(iters):
            for it, (input_nodes, output_nodes, block) in enumerate(train_loader):
                feat = torch.randn(input_nodes.shape[0], in_feat).to(dev)
                h_highmem = conv(block[0], feat, block[0].edata[dgl.ETYPE])
    ...
    with Timer(dev) as t:
        for i in range(iters):
            h_gmm_sorted = conv(block[0], feat, block[0].edata[dgl.ETYPE])
    ...
    with Timer(dev) as t:
        for i in range(iters):
            feat = torch.randn(input_nodes.shape[0], in_feat).to(dev)
            h_gmm_unsorted = conv(block[0], feat, block[0].edata[dgl.ETYPE])
